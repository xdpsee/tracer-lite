...
public abstract class AbstractHandler<T extends Message> extends SimpleChannelInboundHandler<T> implements io.netty.channel.ChannelInboundHandler {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, T msg) {
        ...
        ctx.executor().submit(() -> {
            try {
                checkDevice(ctx, msg);
                Connection connection = (Connection) ChannelAttributes.get(ctx.channel(), ChannelAttributes.CONNECTION);
    ...
    private void checkDevice(ChannelHandlerContext ctx, T msg) throws DeviceException.NotFound, DeviceException.Expired {
